...
internal fun <T : Node, R : Node> parseRepeatLoop (sub : LazyParser<T>, children : Chain<T>?, count : Int, len : Int, ctx : ParseContext, currentPos : Int, startPos : Int, next : ParseNext<R>, nodeClass: Class<T>, type : (Int, Int, Array<T>) -> R) : ParseContState {
    assert (count > 0)
...
fun <T : Node, R : Node> repeat (sub : LazyParser<T>, min : Int, max : Int, nodeClass: Class<T>, type : (Int, Int, Array<T>) -> R) : LazyParser<R> {
    assert(min <= max)
...
inline fun <reified T : Node, R : Node> repeat (noinline sub : LazyParser<T>, min : Int, max : Int, noinline type : (Int, Int, Array<T>) -> R) : LazyParser<R> =
    repeat(sub, min, max, T::class.java, type)

fun repeat (sub : LazyParser<Node>, min : Int, max : Int) : LazyParser<Sequence> =
    repeat(sub, min, max, Node::class.java, type=::Sequence)
